...
const VIOLATION_LABELS = {
  oversizedPosition: { label: 'Oversized Position', short: 'OVERSIZE',  violationType: 'OVERSIZED_POSITION'   },
  movedSL:           { label: 'Moved Stop Loss',    short: 'MOVED SL',  violationType: 'STOP_LOSS_MOVED'      },
  revengeFlag:       { label: 'Revenge Trade',      short: 'REVENGE',   violationType: 'REVENGE_TRADE'        },
  twoLossRuleBroken: { label: '2-Loss Rule Broken', short: '2-LOSS',    violationType: 'TWO_LOSS_RULE_BROKEN' },
};
...
// ─── Violation detail panel (expanded row) ────────────────────────────────────
function ViolationDetail({ trade, onCompletePunishment, meta }) {
  const [expanded, setExpanded] = useState(false);
  const [pendingModal, setPendingModal] = useState(null); // violationType string
  ...
  return (
    <>
      {/* Punishment modal for pending completions */}
      {pendingModal && (
        <PunishmentModal
          trade={trade}
          violationType={pendingModal}
          meta={meta}
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8, alignItems: 'center' }}>
          ...
          {/* Punishment status badge — clickable if pending */}
          {hasPunishment ? (
            <span style={{
            ...
            <button
              onClick={() => {
                // Open modal for the first unresolved violation type
                const firstType = types[0];
                const vt = VIOLATION_LABELS[firstType]?.violationType;
                if (vt) setPendingModal(vt);
              }}
              style={{
                display: 'inline-flex', alignItems: 'center', gap: 4,
            ...
            >
              ⏳ Punishment pending — click to complete
            </button>
          )}
        </div>

        {/* View completed punishment text */}
        {hasPunishment && (
          <button
  ...
  return (
    <div>
      <div className="page-header">
        <h2>Trade Log</h2>
        <p>{activeAccount.name} — violations are permanently flagged · click ⏳ to complete pending punishments</p>
      </div>
      ...
      <div className="card table-desktop">
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-muted)' }}>No trades found.</div>
        ) : (
                        ...
                        <td>
                          {hasViol ? (
                            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', alignItems: 'center' }}>
                              {violTypes.map(type => (
                                <span key={type} className="violation-flag" style={{ fontSize: 10 }}>
                                  🚩 {VIOLATION_LABELS[type]?.short}
                                </span>
                              ))}
                              <span style={{ fontSize: 10, color: t.punishmentCompleted ? 'var(--green)' : 'var(--amber)', fontWeight: 700 }}>
                                {t.punishmentCompleted ? '✓' : '⏳'}
                              </span>
              ...
              <div key={t.id} className={`trade-card ${hasViol ? 'trade-card--violation' : ''}`} onClick={() => hasViol && toggleRow(t.id)}>
                ...
                {hasViol && (
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 10, color: t.punishmentCompleted ? 'var(--green)' : 'var(--amber)', fontWeight: 700 }}>
                      {t.punishmentCompleted ? '✓ Done' : '⏳ Tap to complete'}
                    </span>
