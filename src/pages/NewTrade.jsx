...
export default function NewTrade() {
  ...
  const { activeAccount, meta, trades, checkins, addTrade, updateTrade, refresh } = useAccount();
  const dateStr = today();
  ...
  const [form, setForm] = useState({ pair: '', direction: 'Buy', entry: '', stopLoss: '', takeProfit: '', riskAmount: '', result: 'Win', pnl: '', notes: '', movedSL: null, revengeFlag: null });
  ...
  function proceedChecklist() {
    ...
    setError(''); setChecklistDone(true);
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setSavedTrade(saved);
      const queue = [];
      if (twoLossRuleBroken) queue.push({ trade: saved, violationType: 'TWO_LOSS_RULE_BROKEN' });
      if (oversizedPosition) queue.push({ trade: saved, violationType: 'OVERSIZED_POSITION' });
      if (movedSL)           queue.push({ trade: saved, violationType: 'STOP_LOSS_MOVED' });
      if (revengeFlag)       queue.push({ trade: saved, violationType: 'REVENGE_TRADE' });
      if (queue.length > 0) setPunishmentQueue(queue);
      else setSaved(true);
    } catch (err) {
    ...
    if (current?.trade?.id) {
      try {
        await updateTrade(current.trade.id, {
          ...
          punishmentRecord: punishmentRecord || { violationType: current.violationType, completedAt: new Date().toISOString() },
        });
      } catch {}
    }
    const remaining = punishmentQueue.slice(1);
    setPunishmentQueue(remaining);
    if (remaining.length === 0) setSaved(true);
  }
    ...
    return (
      <div>
        ...
        <div className="alert alert-red" style={{ maxWidth: 500 }}>
          ...
          <div style={{ marginTop: 12 }}><button className="btn btn-primary btn-sm" onClick={() => navigate('/checkin')}>Go to Check-In →</button></div>
        </div>
      </div>
    );
  }
  ...
  if (punishmentQueue.length > 0) {
    ...
    return (
      <>
        <div style={{ filter: 'blur(2px)', pointerEvents: 'none', opacity: 0.3 }}><div className="page-header"><h2>Log Trade</h2></div></div>
        <PunishmentModal trade={current.trade} violationType={current.violationType} meta={meta} onDismiss={handlePunishmentDismiss} />
      </>
    );
  }
    ...
    return (
      <div>
        ...
        <div className={`alert ${hadViolation ? 'alert-amber' : 'alert-green'}`} style={{ maxWidth: 500, marginBottom: 20 }}>
          {hadViolation ? '⚠️ Trade logged with violations. Punishment completed and recorded.' : '✅ Trade logged successfully. Clean execution.'}
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button className="btn btn-primary" onClick={() => { setSaved(false); setChecklistDone(false); setAnswers({}); setSavedTrade(null); setForm({ pair:'', direction:'Buy', entry:'', stopLoss:'', takeProfit:'', riskAmount:'', result:'Win', pnl:'', notes:'', movedSL:null, revengeFlag:null }); }}>Log Another Trade</button>
  ...
  return (
    <div>
      <div className="page-header"><h2>Log Trade</h2><p>{formatDate(dateStr)} · {activeAccount.name}</p></div>
      ...
      {!checklistDone && (
        <div className="card" style={{ maxWidth: 580 }}>
          ...
          {!hasBlocker() ? (
            <button className="btn btn-primary" style={{ width: '100%', marginTop: 8 }} onClick={proceedChecklist} disabled={!allAnswered()}>Proceed to Trade Entry →</button>
          ) : (
        ...
        <div className="card" style={{ maxWidth: 580 }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-row form-row-3" style={{ marginBottom: 16 }}>
              <div className="form-group" style={{ marginBottom: 0 }}><label>Entry Price</label><input type="number" step="any" placeholder="0.00" value={form.entry} onChange={e => handleChange('entry', e.target.value)} required /></div>
              <div className="form-group" style={{ marginBottom: 0 }}><label>Stop Loss 🔒</label><input type="number" step="any" placeholder="0.00" value={form.stopLoss} onChange={e => handleChange('stopLoss', e.target.value)} required /><div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>Locked after saving</div></div>
              <div className="form-group" style={{ marginBottom: 0 }}><label>Take Profit</label><input type="number" step="any" placeholder="0.00" value={form.takeProfit} onChange={e => handleChange('takeProfit', e.target.value)} /></div>
            </div>
            <div className="form-row form-row-2" style={{ marginBottom: 16 }}>
              <div className="form-group" style={{ marginBottom: 0 }}>
                <label>Risk Amount ($) <span style={{ color: 'var(--text-muted)', fontWeight: 400, textTransform: 'none', letterSpacing: 0, marginLeft: 6 }}>standard: {fmtMoney(meta.maxRiskPerTrade)}</span></label>
                <input type="number" step="0.01" min="0.01" placeholder={fmtMoney(meta.maxRiskPerTrade).replace('$', '')} value={form.riskAmount} onChange={e => handleChange('riskAmount', e.target.value)} required style={{ borderColor: isOversized ? 'var(--red)' : 'var(--border)' }} />
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>Conservative: {fmtMoney(meta.conservativeRisk)} · Standard: {fmtMoney(meta.maxRiskPerTrade)} · Max ever: {fmtMoney(meta.maxRiskEver)}</div>
              </div>
              <div className="form-group" style={{ marginBottom: 0 }}>
                <label>Result</label>
                <div className="radio-group">
                  {['Win', 'Loss', 'Breakeven'].map(r => (
                    <button key={r} type="button" className={`radio-btn ${form.result === r ? (r === 'Win' ? 'selected-yes' : r === 'Loss' ? 'selected-no' : '') : ''}`} style={form.result === r && r === 'Breakeven' ? { background: 'rgba(59,130,246,0.15)', borderColor: 'var(--blue)', color: 'var(--blue)' } : {}} onClick={() => handleChange('result', r)}>{r}</button>
                  ))}
                </div>
              </div>
            </div>
            {isOversized && <div className="alert alert-red" style={{ marginBottom: 16 }}><strong>⚠️ You are risking more than your max ({fmtMoney(meta.maxRiskPerTrade)}).</strong><br />This trade will be saved as a <strong>VIOLATION — OVERSIZED_POSITION</strong>.</div>}
            <div className="form-group"><label>P&L ($)</label><input type="number" step="0.01" placeholder="e.g. 8.50 or -6.00" value={form.pnl} onChange={e => handleChange('pnl', e.target.value)} required /></div>
            <div className="form-group"><label>Notes</label><textarea placeholder="What happened? What did you see? What would you do differently?" value={form.notes} onChange={e => handleChange('notes', e.target.value)} /></div>
            ...
            <div className="checklist-item" style={{ borderColor: form.movedSL === 'yes' ? 'var(--red)' : form.movedSL === 'no' ? 'var(--green)' : 'var(--border)' }}>
              <div style={{ flex: 1 }}>
                ...
                {form.movedSL === 'yes' && <div className="alert alert-red" style={{ marginTop: 10 }}>🚩 Permanently flagged. Punishment required after saving.</div>}
              </div>
            </div>
            <div className="checklist-item" style={{ borderColor: form.revengeFlag === 'yes' ? 'var(--red)' : form.revengeFlag === 'no' ? 'var(--green)' : 'var(--border)' }}>
              <div style={{ flex: 1 }}>
                ...
                {form.revengeFlag === 'yes' && <div className="alert alert-red" style={{ marginTop: 10 }}>🚩 Permanently flagged. Punishment required after saving.</div>}
              </div>
            </div>
            {error && <div className="alert alert-red" style={{ margin: '12px 0' }}>{error}</div>}
            <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
              <button type="submit" className="btn btn-primary" style={{ flex: 1 }} disabled={saving}>
                {saving ? 'Saving…' : (isOversized || form.movedSL === 'yes' || form.revengeFlag === 'yes') ? 'Save Trade (Violations Will Be Flagged)' : 'Save Trade'}
              </button>
