...
import {
  today, getCheckinForDate, saveCheckin,
  formatDate, fmtMoney, calcAccountMeta,
} from '../store';
...
export default function CheckIn() {
  ...
  // Guard — should not happen due to onboarding gate, but be safe
  if (!activeAccount || !meta) {
    return (
      <div className="alert alert-red" style={{ maxWidth: 500 }}>
        <strong>No account selected.</strong> Please set up an account first.
  ...
  const existing = getCheckinForDate(activeAccount.id, dateStr);
  ...
  const realCapitalPct = realCapitalRemaining !== null && meta.realCapital > 0
    ? ((realCapitalRemaining / meta.realCapital) * 100).toFixed(1)
    : null;
  ...
  function handleSave(e) {
    e.preventDefault();
    if (!balance || !moodWord.trim()) return;
    if (!canProceed) return;

    saveCheckin({
      id: existing?.id || (activeAccount.id + '_' + dateStr),
        ...
        <form onSubmit={handleSave}>
          ...
          <div className="form-group">
            ...
            <input
              type="number"
              step="0.01"
              placeholder={`e.g. ${activeAccount.startingBalance?.toFixed(2) || '6000.00'}`}
              value={balance}
          ...
          {balanceNum !== null && (
            <div className="card" style={{ background: 'var(--surface2)', marginBottom: 16, padding: 14 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                ...
                <span style={{ fontWeight: 800, color: realCapitalRemaining >= 0 ? 'var(--green)' : 'var(--red)' }}>
                  {fmtMoney(realCapitalRemaining)}
                </span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                ...
                <span style={{
                  fontWeight: 800,
                  color: realCapitalPct >= 50 ? 'var(--green)' : realCapitalPct >= 20 ? 'var(--amber)' : 'var(--red)'
                }}>
                  {realCapitalPct}%
                </span>
              </div>
              <div className="progress-bar">
                <div
                  ...
                  style={{
                    width: `${Math.max(0, Math.min(100, realCapitalPct))}%`,
                    background: realCapitalPct >= 50 ? 'var(--green)' : realCapitalPct >= 20 ? 'var(--amber)' : 'var(--red)'
                  }}
                />
              </div>
              ...
              <input
                ...
                style={{
                  background: `linear-gradient(to right, ${
                    mood <= 2 ? 'var(--red)' : mood === 3 ? 'var(--amber)' : 'var(--green)'
                  } ${(mood - 1) * 25}%, var(--surface2) ${(mood - 1) * 25}%)`
                }}
              />
          ...
          {isHighRisk && (
            <div className="alert alert-red" style={{ marginBottom: 16 }}>
              ...
              Consider making today a no-trade day. Your emotional state is not optimal for trading.
          ...
          <div className="form-group">
            ...
            <input
              type="text"
              placeholder="e.g. Sharp, Tired, Confident..."
              value={moodWord}
          ...
          <button
            ...
            disabled={!canProceed || !balance || !moodWord.trim()}
          >
            {saved ? 'Update Check-In' : 'Complete Check-In'}
          </button>
        </form>
      </div>

      {/* Account risk reference */}
