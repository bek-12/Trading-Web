...
import {
  today, getJournalByAccount, saveJournalEntry, getJournalForDate,
  getCheckinsByAccount, formatDate,
} from '../store';
import {
...
export default function Journal() {
  ...
  const entries = getJournalByAccount(activeAccount.id).sort((a, b) => b.date.localeCompare(a.date));
  const checkins = getCheckinsByAccount(activeAccount.id).sort((a, b) => a.date.localeCompare(b.date));

  const moodData = checkins.slice(-30).map(c => ({
    date: c.date,
    mood: c.mood,
    word: c.moodWord,
  }));
  ...
  return (
    <div>
      <div className="page-header">
        <h2>Journal</h2>
        <p>{activeAccount.name} — daily reflections and mood history</p>
      </div>
      ...
      <div className="grid grid-2" style={{ gap: 20, alignItems: 'start' }}>
        ...
        <div>
          ...
          <div className="card">
            ...
            {entries.length === 0 ? (
              <div style={{ color: 'var(--text-muted)', fontSize: 13 }}>No journal entries yet.</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {entries.map(e => (
                  <button key={e.date} className="btn btn-ghost btn-sm"
                    style={{
                      justifyContent: 'flex-start', textAlign: 'left',
                      background: viewDate === e.date ? 'var(--surface2)' : 'transparent',
                      borderColor: viewDate === e.date ? 'var(--accent)' : 'var(--border)',
                    }}
                    onClick={() => setViewDate(e.date)}>
        ...
        <div>
          {viewEntry && viewDate !== dateStr && (
            <div className="card" style={{ marginBottom: 16 }}>
              ...
              <div style={{ color: 'var(--text-dim)', lineHeight: 1.7, whiteSpace: 'pre-wrap', fontSize: 14 }}>
                {viewEntry.text}
              </div>
            </div>
          )}
          ...
          <div className="card">
            ...
            {moodData.length < 2 ? (
              <div style={{ color: 'var(--text-muted)', fontSize: 13 }}>
                Complete more daily check-ins to see your mood trend.
              </div>
            ) : (
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={moodData} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(val) => [MOOD_LABELS[val], 'Mood']} />
            ...
            {checkins.length > 0 && (
              <div style={{ marginTop: 16 }}>
                <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 8, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  Recent Check-Ins
                </div>
